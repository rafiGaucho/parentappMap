...
import {
  ...
  StyleSheet,ImageBackground,TextInput,AsyncStorage,TouchableWithoutFeedback
} from 'react-native';
import {connect} from 'react-redux';
import {Icon} from 'native-base';
import Hr from "react-native-hr-component";
import {userLogin,saveUser,toForgotPassword,toSignup} from './../store/session/actions.js'
        ...
        .then(value => {
          ...
          if(value.success ===true){
            ...
            value.studentList.filter(item=>{
              let valId = item.busRouteId;
              let valCode = item.busRouteCode;
              let valName = item.busRoute;
              busRouteId.push(valId);
              busRouteCode.push(valCode);
              busRoute.push(valName);
            })
          ...
          <View style={{flex:1,justifyContent:'space-around'}}>
            ...
            <View style={{flexDirection:'row',alignItems:'center',justifyContent:'center'}}>
              <TouchableWithoutFeedback onPress={this.handleForgotPassword}>
                <Text style={{fontWeight:'400',}}>Forgot Password ?</Text>
...
toForgotPassword:toForgotPassword,toSignup:toSignup
}
